...
def _find_data():
    ...
    explicit = [
        os.path.join(home, "Desktop", "CreditFair", "Week-1", "dataset", "german.data"),
        os.path.join(home, "Desktop", "CreditFair", "Week 1", "dataset", "german.data"),
    ]
    for p in explicit:
        if os.path.exists(p):
    ...
    patterns = ["german.data", "german.data.csv",
    ...
    pytest.skip("german.data not found. Set GERMAN_DATA=/path/to/german.data "
                "or place german.data in the project (searched project tree "
                "and ~/Desktop/CreditFair/Week-1/dataset).")
