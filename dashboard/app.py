"""CreditFair — live dashboard (Streamlit).

Sits directly on the tested module `src/creditfair.py`. Six tabs:
Overview · Fairness monitor · Review queue · Applicant explainer · Live scorer · Compliance.

Run:  streamlit run app.py      (from the project root, with german.data present)
"""
import os
import sys
import numpy as np
...
# import creditfair whether it sits next to this file (flat) or in ../src (nested project)
for _p in (os.path.join(_HERE, "src"), os.path.join(_HERE, "..", "src"),
           os.path.join(_HERE, ".."), _HERE):
    _p = os.path.abspath(_p)
    if os.path.isdir(_p) and _p not in sys.path:
        sys.path.insert(0, _p)
import creditfair as cf
